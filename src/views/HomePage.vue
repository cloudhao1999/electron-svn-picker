...
const generateNewFold = () => {
  remote.dialog
    ...
    .then((result: any) => {
      ...
      copyFile(
        convertObjToArray(globalRecordFileMap.value),
        basePath.value,
        filePath,
        "./new/",
        formData.value.svnPath
      );
      ElMessage({
        message: "文件生成成功！",
        type: "success",
...
const handleSplitRecord = async (formEl: FormInstance | undefined) => {
  ...
  return await formEl.validate((valid, fields) => {
    if (valid) {
      debugger;
      const { fileStr, recordFileMap } = splitRecord(
...
<template>
  ...
  <div v-if="active === 0">
    <div class="step-btn">
      ...
      <el-button :disabled="projectFileList.length === 0" @click="nextStep"
        >下一步</el-button
      >
    </div>
    ...
    <div class="mt-5 mb-5">
      ...
      <el-button-group class="ml-3 mr-3" type="primary">
        <el-button
          plain
          @click="handleCopy(0)"
          :disabled="multipleSelection.length === 0 || !isCopyMod"
          >复制修改路径</el-button
        >
        <el-button
          plain
          @click="handleCopy(1)"
          :disabled="multipleSelection.length === 0 || !isCopyAdd"
          >复制新增路径</el-button
        >
        <el-button
          plain
          @click="handleCopy(2)"
          :disabled="multipleSelection.length === 0 || !isCopyDel"
          >复制删除路径</el-button
        >
      </el-button-group>
      <el-button @click="clear" :disabled="multipleSelection.length === 0"
        >一键清空</el-button
      >
    </div>
    <el-form>
